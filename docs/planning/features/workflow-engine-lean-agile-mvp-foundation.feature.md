...
| 05  | Secure runtime and container proof        | Encrypted PouchDB runtime-store design, secret injection, public baseline fixture, backup/restore, and image-content audit           | 02–04      | Image inspection proves no live data/key; runtime recovery works from an approved encrypted store.    |
| 06  | Activity projection contract              | Per-principal/per-organization projection, retention window, gateway SSE contract, authorization tests, and replay checkpointing     | 02–04      | One principal receives only their filtered activity, including after reconnect/replay.                |
| 07  | Representative foundation pilot           | Three-session Portfolio workflow pilot using a synthetic/disposable principal and workspace; diagnostics and recovery report         | 01–06      | Deterministic resume, audit, macro replay, projection repair, and redaction all demonstrated.         |
| 08  | Second-consumer extraction decision       | Joint Portfolio/Singularity evidence review, adapter/extraction decision, migration plan or explicit deferral                        | 07         | Decision records whether Portfolio justifies a package/public adapter; no extraction is assumed.      |
...
| ----------------------------------- | ------------------------------------------------------------------------------- | --------------- |
| Domain unit tests                   | Workflow and macro invariants, allowed transitions, immutable versions          | 01–04           |
| PouchDB infrastructure tests        | Idempotency, redaction, indexes, snapshots, restore, checkpoint replay          | 02–06           |
| Cucumber/Screenplay domain assembly | Lean-Agile MVP gates, macro approval/replay, blocked recovery                   | 01–04, 07       |
| HTTP/SSE assembly                   | Authorization scope, filtered projection, reconnect/replay                      | 06–07           |
| Container smoke test                | Public image contents, injected secret path, runtime PouchDB recovery           | 05, 07          |
| Contract tests                      | No private imports; compatible public contract across Portfolio and Singularity | 00–08           |
